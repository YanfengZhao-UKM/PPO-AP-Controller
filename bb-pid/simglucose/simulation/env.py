...
def myreward(BG_last_hour):
    if len(BG_last_hour) < 1:
        ...
        if 300 < bg_current <= 600:
            return -5*risk_current
        elif 180 < bg_current <= 300:
            return -2*risk_current
        elif 70 < bg_current <= 180:
            return -risk_current
        elif 50 < bg_current <= 70:
            return -3*risk_current
        elif 10 < bg_current <= 50:
            return 10*risk_current
        else:
            return -100000

def simpreward(BG_last_hour):
    if len(BG_last_hour) < 1:
        return 0
    else:
        bg_current=BG_last_hour[-1]
        if bg_current >= 70 and bg_current <= 180:
            reward = 1
        elif bg_current<=10 or bg_current >600:
            reward = -100
        else:
            reward = -1
        return reward
...
class T1DSimEnv(object):
    ...
    def step(self, action, reward_fun=simpreward):
        '''
