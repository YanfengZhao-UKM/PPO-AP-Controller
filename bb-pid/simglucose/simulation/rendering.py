...
class Viewer(object):
    ...
    def initialize(self):
        ...
        axes[0].axhspan(180, 300, alpha=0.3, color='red', lw=0)
        axes[0].axhspan(300, 1000, alpha=0.3, color='darkred', lw=0)
    ...
    def render(self, data):
        ...
        self.lines[4].set_ydata(data['LBGI'].values*0.9)

        self.lines[5].set_xdata(data.index.values)
        self.lines[5].set_ydata(data['HBGI'].values*1.05)

        self.lines[6].set_xdata(data.index.values)
        self.lines[6].set_ydata(data['Risk'].values)
        #print(data['LBGI'].values,data['HBGI'].values)
