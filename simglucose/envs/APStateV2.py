...
class APState:
    def __init__(self, observation):
        ...
        self.currentstate = np.concatenate(
            (self.observation_norm, self.observation_dt_norm, self.observation_ddt_norm, self.insulin_hist_norm,
             self.cho_hist_norm),
    ...
    def merge(self, observation, insulin, cho):
        ...
        self.cho = cho
        self.cho_hist = np.roll(self.cho_hist,1)  # cycle planed meal array by 1 position move the oldest value from the rightmost position to the first position
        self.cho_hist[0] = self.cho  # replace first value in array (aka - the oldest value) with the most recent action
